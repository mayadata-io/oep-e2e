...
<b>tcid:</b> iuod28 <br>
<b>name:</b> "Upgrade the application with 3 replicas when one replica pod is not running"<br>
...
<table>
  ...
  <tr>
    ...
    <td> Upgrade the application with 3 replicas when one replica pod is not running </td>
    <td> GKE </td>
  </tr>
</table>
...
- Now create some scenario so that the application volume replica pod should go in pending state .

- After sending application volume replica pod to pending state try to upgrade `Data Plane` components.
